...
|----------|-----------|--------|
| ~~elastix Affine+Spline (REG-05 KEEP)~~ → **fully manual registration** | **REVISED 2026-07-30 (operator, M3 Hipp2, all 6 sections).** Chain is now DeepSlice → Review/positioning (position + tilt) → **manual affine** in the Affine tab (angle/X/Y/scale, matched to the **Label Borders** overlay) → **manual BigWarp** in the Spline tab → export. **elastix is out of the chain.** Reverses the 2026-07-20 REG-05 "KEEP elastix" decision under that decision's own rule (operator visual quality decides; time explicitly irrelevant). Note the overlay choice inverts *consistently*, not contradictorily: an intensity optimizer needs Nissl's intensity correspondence and fails on a line drawing, whereas a human aligns boundaries and therefore wants Label Borders. elastix stays installed/pinned (5.2.0) for any future use, where the Nissl-fixed-channel rule still holds. | **Current** |
...
| 260728-nus | cockpit increment 1: `scripts/cockpit_checks.py` (6 QC gates, each value+PASS/FLAG; headless QuPath cmd builder; comment-preserving BraiAn.yml param lock) + thin `notebooks/01_calibrate.ipynb` / `02_batch.ipynb`. Gates read regions.tsv (QuPath's own per-region count+area rollup) so the nested-annotation denominator bug is structurally impossible; atlas_cells.tsv is unusable for region work (region_label = detection class). Fixed marker-name case mismatch (pipeline.yml `Fos` vs export `fos_bgsub`) that silently N/A'd every marker gate. Self-test 10/10 sections; both fixtures run 0 errors; P(Fos+\|TdT+)=42.4% reproduces known wBA1 result. Both wBA fixtures FLAG white-matter (cc~4.8k vs cortex~4.0k/mm²) + ventricle — real detection-quality findings. Follow-ups: `advisory_gates` knob parks white-matter+ventricle as advisory (9618458 — DAPI densities untrustworthy while parked, ratio readouts fine); nucleus-area band lower bound 50→25 µm² as the new default (fee7a1a — operator call, M3-derived band never matched thin-Z images). Both fixtures end 5/5 PASS | 2026-07-28 | 0ccde36, 49da663, 9618458, fee7a1a | [260728-nus-cockpit-checks-notebooks](./quick/260728-nus-cockpit-checks-notebooks/) |
...
| 260729-f03 | cockpit increment 3 (final): `scripts/cockpit_animal.py` — animal-level rollup (pool counts/areas across slices, recompute every ratio, never mean-of-ratios), D-1 tagged/activity role resolution from compartment (not marker declaration order — proven on M3's Fos-then-TdT declared order), full 8-column overlap metric family (tagging/activity/reactivation/reverse rate, overlap_above_chance, log2_odds_ratio(_hc), jaccard), D-2 fail-loud marker-set stacking (`--stack-on-intersection` opt-in), D-3 explicit group mapping, D-4 classifiable-vs-anchor N source, LONG + Prism WIDE CSV export + thin `notebooks/03_animal.ipynb`. Self-test proves the anti-averaging assertion (pooled ratio != mean of per-slice ratios) plus 25 other checks; both real fixtures validated (M3 2-marker full metric family, wBA tdt-only emits tagging_rate only); mismatched-marker-set run correctly exits non-zero; notebook runs 0 errors against M3. Cockpit is now feature-complete (3/3 increments) | 2026-07-29 | c2e59a6, 38dc36e, 601eab9 | [260729-f03-cockpit-increment-3-of-3-animal-level-ro](./quick/260729-f03-cockpit-increment-3-of-3-animal-level-ro/) |
| 260729-qc8 | operator-tunable parameters from one control surface: 4 previously-unreachable `czi_mip.py` knobs on the CLI (`--feather-margin`, `--no-flat-field`, `--dapi-z`, `--scenes`) + new `scripts/cockpit_tune.py` (`--list`/`--set`/`--log`) + `docs/runbook/03-tuning.md` + notebook sections 1b/7. Configs stay SPLIT — D-14 is now asserted in self-test case (g) (a detection knob can never route to pipeline.yml, nor a classification knob to BraiAn.yml), not just documented. **ruamel.yaml is NOT in the braian env** and a PyYAML round-trip deletes every comment in BraiAn.yml/pipeline.yml — the comments that carry the tuning rationale — so the writer reads with PyYAML and writes by targeted line-level scalar substitution; verified on the real configs (3 knobs → exactly 3 lines changed, all comments intact, line count and trailing-comment column preserved). `--scenes` cuts a tuning iteration from ~30 min (6-scene series) to ~5 min (one scene) and makes crash recovery free. Omitting all four czi_mip flags is byte-identical to prior behavior (self-test case j), so MIPs cut before/after stay comparable. MIP knobs are listed but `--set` refuses them (they are CLI args with nowhere to persist). Deliberately NOT built: hand-count/ground-truth machinery — operator validates by eye | 2026-07-30 | b20fa68, 78c9ee2, a06c2c6 | [260729-qc8-make-pipeline-parameters-operator-tunabl](./quick/260729-qc8-make-pipeline-parameters-operator-tunabl/) |
| 260729-gme | cockpit increment 3 follow-up: interpretation plots in `cockpit_animal.py` — 5 figures (region ranking on a log2(overlap_above_chance) display transform, raw-vs-corrected dual panel, evidence guard vs `min_cells`, per-slice pooled-vs-mean spread proving D-8's anti-averaging rule, L/R hemisphere symmetry QC) + `build_figures`/`save_figures` + `--plots` CLI flag + thin section 5 in `notebooks/03_animal.ipynb`. Self-test 65/65 PASS incl. subprocess backend-unchanged (bd5d11f guard), 1-marker skip path, all-zero-region exclusion, role-correct reactivation, anti-averaging. Both fixtures run 0 errors (M3 275-region unfiltered writes 5 PNGs, wBA tdt-only writes 2 + expected skips). Mandatory visual pass found + fixed 4 real layout bugs (title/annotation collision, disproportionate margins, ~100-label figure-5 crowding, notebook-inline axis-label clipping). Finding (not a bug): CA1 ranks #42/275 unfiltered (buried behind low-N fiber-tract noise — exactly what evidence_guard exists to flag) but #1 at ~4.2x on the curated hippocampal region set, matching the recorded result | 2026-07-29 | 4e4d703, 550ac41, 90aafdf | [260729-gme-cockpit-increment-3-follow-up-interpreta](./quick/260729-gme-cockpit-increment-3-follow-up-interpreta/) |
